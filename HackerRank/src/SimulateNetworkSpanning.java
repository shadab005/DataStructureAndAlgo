...
public class SimulateNetworkSpanning {
	...
	public static void main(String[] args) {
		...
		Comparator<Edge> byEdgeWeight = (o1, o2) -> o1.weight() - o2.weight();
		...
		while (!pq.isEmpty() && edgeCount < g.n() - 1) {
			Edge e = pq.remove();
			int v = e.either();
			int w = e.other(v);
		...
		// for (Edge z : mst)System.out.println(z);
		long sum = 0;
		int k = 0;
		for (int i = mst.length - 1; i >= 0; i--) {
			if (k < c.length && mst[i].weight() > c[k]) {
				sum += c[k];
				k++;
			} else {
				sum += mst[i].weight();
